...
that is designed to address several issues
found in the original Single Shot Detector (SSD)~\cite{liu2016ssd}.
The network architecture is illustrated in Figure~\ref{fig:ppn}.
There are two major changes to the original SSD:
(1) the box predictor is shared across feature maps with different scales;
(2) the convolutions between feature maps are replaced with the max pooling operations.
...
\subsection{Sharing Box Predictor}
The original SSD uses separate box predictors for feature maps of different scales.
While each box predictor is allocated to spend its full capacity on one specific scale,
this design could be vulnerable in some practical situations.

One problem raises when the training data has inbalanced groundtruth bounding boxes with different sizes.
Because each box predictor only sees a portion of the groundtruth boxes
whose sizes corresponds to the scale of one specific predictor,

Because each box predictor only sees a portion of groundtruth boxes
whose sizes corresponds to the predictor's scale,
the predicted scores can be mis-calibrated across different predictors.
corresponding to its own scale,
the predicted scores can be mis-calibrated across different scales. 
...
this could become 
this could also become less robust 


we have found this design may lead to several problems in practice.
...
While each box predictor focuses on one specific scales
and spends full capacity specializing 
...
\subsection{Comparing with SSD and FPN}
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.8\linewidth]{figure/eg.pdf}
\end{center}
